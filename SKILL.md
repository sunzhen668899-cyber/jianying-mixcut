...
```
读 `work/probe/` 里每个源片的 3 张抽帧，**肉眼定位烧录字幕的上边界**，写回 `config.json` 的 `sources.<文件>.band`（如 `[0.78, 0.93]`；源片无烧录字幕则 `null`）。band[0] 是裁剪线：要比字幕最上沿再高 1~2%，否则字幕上沿会残留在画面里。若源片**顶部也有烧录内容**（作者名、标题条、平台水印），再加 `sources.<文件>.top`（如 `0.18`）做上裁，prep 会同时裁掉顶部 top 以下、底部 band[0] 以上的画面。同时把产品名/型号等填入 config。
...
- 源片烧录花字（非底部字幕带，而是画面中部的大字卖点贴字，如「增压丝雨水」）顶沿可能高于 band[0] 裁剪线，prep 裁不掉：精剪选段时先用 probe/QC 帧标出各源片的花字区间，选段直接避开，或把该卖点段换到别的源片（36888 踩过：1.mp4 18~25s 花字段换用 3.mp4 同卖点段）
- 纯音乐无人声源片会让 whisper 产出幻听稿（一堆重复短句如「谢谢大家」）：段数异常多 + 文本高度重复即为信号，用 `--no-vad` 免 VAD 重转写复核确认后，该片按纯画面素材用（只取画面段，文案自写），不要拿幻听稿做语义去重依据
- 源片顶部+底部都有烧录内容时用 `top`+`band` 双裁（ZQ6640 踩过：2.mp4 顶部 18% 是作者标题条、底部 82% 起是字幕带，配 `"top": 0.18, "band": [0.82, 0.95]`）；双裁后还要全时段抽帧复查——裁不掉的残留（如画面中部「加速中」红字）标出时间区间，精剪选段避开
